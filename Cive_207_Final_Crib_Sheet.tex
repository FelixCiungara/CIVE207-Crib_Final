...
\node[opacity=0.7, anchor=north east] (image) at (\linewidth,0.0cm) {\includegraphics[width=0.80in]{image1.png}};
\end{tikzpicture}
...
\raisebox{-0.4\height}{\includegraphics[width=0.65in]{image2.png}}
\end{tabular}

\vspace{3pt}
\begin{tabular}{@{}ll@{}}
$\tau = \dfrac{T\rho}{J}$ &
\raisebox{-0.4\height}{\includegraphics[width=0.5in]{image3.png}}
\end{tabular}
...
{\footnotesize\begin{tabular}{@{}p{0.3\linewidth}p{0.6\linewidth}@{}}
$0 \le \rho \le \rho_Y$: & $\rho_Y \le \rho \le c$: \\[3pt]
{\scriptsize$\tau = \dfrac{\tau_Y}{\rho_Y}\rho$} &
{\scriptsize$\tau = \tau_Y$} \\[6pt]
\end{tabular}}

$T = \dfrac{4}{3}T_Y\left(1 - \dfrac{1}{4}\dfrac{\rho_Y^3}{c^3}\right)$\\
$T = \dfrac{4}{3}T_Y\left(1 - \dfrac{1}{4}\dfrac{\rho_Y^3}{c^3}\right)$\\
$T_P = \dfrac{4}{3}T_Y$


$\dfrac{\rho_Y}{c} = \dfrac{\phi_Y}{\phi}$ \quad ($\phi_Y$ = angle when outer starts yielding)

\vspace{-4pt}
\sectiontitleOld{Torsion --- Rectangular Cross-Section}
\vspace{-10pt}
\[
...
\sectiontitleOld{Hollow Cylinder (Elasto-Plastic)}

{\footnotesize\[
T = \iint \rho\,\tau\,dA = 2\pi\int_{a}^{b}\tau\,\rho^2\,d\rho
\]}
\[
T_e = \frac{\tau_{\max} J}{c}
\]

{\footnotesize\[
T_{e\text{-}p} = 2\pi\tau_Y\!\left(\dfrac{b^3}{3} - \dfrac{a^4}{4\rho_Y} - \dfrac{\rho_Y^3}{12}\right)
\]}

\[
T_p = \frac{2}{3}\pi\tau_Y\!\left(b^3 - a^3\right)
\]
...
{\footnotesize\[
M_Y = \frac{I}{c}\,\sigma_Y
\]}

{\footnotesize\begin{tabular}{@{}p{0.28\linewidth}p{0.68\linewidth}@{}}
$0 \le |y| \le y_Y$: &
$y_Y \le |y| \le c$: \\[3pt]
$\sigma = -\dfrac{\sigma_Y}{y_Y}y$ &
$M = \dfrac{3}{2}M_Y\!\left(1 - \dfrac{y_Y^2}{3c^2}\right)$ \\[6pt]
 & $M_P = \dfrac{3}{2}M_Y$
\end{tabular}}
\vspace{2pt}
\hrule height 0.3pt
...
{\footnotesize\begin{tabular}{@{}l@{\hspace{2pt}}l@{}}
$M_e = \dfrac{\sigma_{\max} I}{c}$ & (elastic) \\[6pt]
$M_{e\text{-}p} = b\sigma_y\!\left(c^2 - \dfrac{1}{3}y_y^2\right)$ & (elasto-plastic) \\[6pt]
$M_p = b\sigma_y c^2$ & (fully plastic)
\end{tabular}}

\vspace{2pt}
$Z = \dfrac{M_p}{\sigma_Y}$ \quad (plastic section modulus)
...
\raisebox{-0.4\height}{\includegraphics[width=0.5in]{image10.png}}
\end{tabular}
...
\begin{tikzpicture}[remember picture, overlay]
\node[opacity=0.7, anchor=north east] (image) at (\linewidth+1cm,1.5cm) {\includegraphics[width=1.0in]{image12.png}};
\end{tikzpicture}
\begin{minipage}{0.95\linewidth}
...
\begin{center}
\includegraphics[width=1.2in]{image11.png}
\end{center}
...
\sectiontitleOld{NEW -- Residual Curvature}
\[\frac{1}{\rho_{plastic}} = \frac{\varepsilon_{x,Yield}}{y_{Yield}}\]
\[\frac{1}{\rho_{unload}} = \frac{M}{EI}\]
\[\frac{1}{\rho_r} = \frac{1}{\rho_{plastic}} - \frac{1}{\rho_{unload}}\]
...
Fixed support: $y = 0$, \quad $y' = 0$\\
Free End: $M = EIy'' = 0$, \quad $V = EI y''' = 0$\\
Segments: $y_1(a) = y_2(a)$, \quad $y_1'(a) = y_2'(a)$\\
Symmetry: $y(\text{center}) = y_{\max}$, \quad $y'(\text{center}) = 0$\\
...
$\tau = \frac{VQ}{It}$ &
\raisebox{-0.4\height}{\includegraphics[width=0.65in]{image15.png}}
\end{tabular}


$V$: shear force, $Q$: first moment of area\\
$I$: second moment of inertia, $t$: width
...
\vspace{-2pt}
\[
F_{\text{nail}} = \frac{q \cdot s}{n} \quad \text{\scriptsize $\left(\frac{\text{Force}}{\text{Length}} \cdot \frac{\text{Length}}{\text{Nail}} \div \text{rows} = \frac{\text{Force}}{\text{Nail}}\right)$}
\]
...
\begin{center}
\raisebox{-.5\height}{\includegraphics[width=0.6in]{image16.png}} \hspace{10pt} \raisebox{-.5\height}{\includegraphics[width=0.6in]{image26.png}}
\end{center}
\vspace{-5pt}

\begin{center}
\raisebox{-.5\height}{\includegraphics[width=1in]{image17.png}} \hspace{10pt} \raisebox{-.5\height}{\includegraphics[width=1in]{image18.png}}
\end{center}
\vspace{-5pt}
...
\\[3pt]
\text{$\theta_{p1}$ brings the $x$ face to $\sigma_{\max}$}
...
\\[3pt]
\text{$\theta_{p1}$ brings the $x$ face to $\varepsilon_{\max}$}
...
\filldraw[black](\cAB,\rAB) circle(0.18)
  node[anchor=south,font=\tiny]{$\tau_{\text{abs,max}}$};
% Label above diagram
\node[anchor=south,font=\tiny] at (\cAB,\h+12.0cm) {One $\sigma$ compressive};
...
\begin{tikzpicture}[remember picture, overlay]
\node[opacity=1, anchor=north east] (image) at (\linewidth,1.0cm) {\includegraphics[width=1.20in]{image29.png}};
\node[opacity=1, anchor=north east] (image) at (\linewidth,-1.0cm) {\includegraphics[width=0.80in]{image30.png}};
...
\vspace{0.5cm}
\begin{center}
\includegraphics[width=1in]{image34.png}
\end{center}
Tilted cut\\
Shear at top is 0
...
%% COLUMN 3
\sectiontitle{Failure Theories}

% Background images with absolute positioning
\begin{tikzpicture}[remember picture, overlay]
\node[opacity=0.6, anchor=north west] at (0,0) {\includegraphics[width=0.80in]{image19.png}};
\node[opacity=0.6, anchor=north east] at (7.0cm,-3cm) {\includegraphics[width=0.80in]{image20.png}};
\node[opacity=0.6, anchor=north west] at (0, -4.5cm) {\includegraphics[width=0.80in]{image21.png}};
\node[opacity=0.6, anchor=north east] at (7.0cm, -5.5cm) {\includegraphics[width=0.80in]{image22.png}};
\node[opacity=1, anchor=north west] at (0cm, -7cm) {\includegraphics[width=0.80in]{image23.png}};
\node[opacity=0.6, anchor=north east] at (7.0cm, -7cm) {\includegraphics[width=0.80in]{image24.png}};
\end{tikzpicture}
...
\subsectiontitle{Maximum Shearing Stress Criterion (Tresca)}
$\tau_{\max} = \frac{1}{2}\sigma_Y$ \hspace{0.5em} (Tresca, uniaxial tension)\\
$\tau_{\max} = \frac{1}{2}|\sigma_1 - \sigma_2|$ \hspace{0.5em} (plane stress, general)\\
$\tau_{\max} = \frac{1}{2}\sigma_{\max}$ \hspace{0.5em} (plane stress, if $\sigma_{\min}=0$)\\
If $\sigma_a$ and $\sigma_b$ have the same sign: $|\sigma_a| \leq \sigma_Y$ and $|\sigma_b| \leq \sigma_Y$ must hold.\\
If $\sigma_a$ and $\sigma_b$ have opposite signs: $|\sigma_a - \sigma_b| \leq \sigma_Y$ must hold.\\

\vspace{-8pt}

\subsectiontitle{Maximum-Distortion-Energy Criterion (von Mises)}
$u_d = \frac{1}{6G}(\sigma_a^2 - \sigma_a\sigma_b + \sigma_b^2)$ \hspace{0.5em} (distortion energy per unit volume)\\
$\sigma_a^2 - \sigma_a\sigma_b + \sigma_b^2 \leq \sigma_Y^2$ \hspace{0.5em} (von Mises failure criterion)\\
...
\subsectiontitle{Maximum Normal Stress Criterion}
$|\sigma_a| \leq \sigma_U$ and $|\sigma_b| \leq \sigma_U$ \hspace{0.5em} (tension)\\

\vspace{-5pt}

\subsectiontitle{Maximum-Normal-Strain Criterion (Saint-Venant)}
$|\epsilon_a| \leq \epsilon_U$ and $|\epsilon_b| \leq \epsilon_U$ \hspace{0.5em} (tension)\\

\vspace{-8pt}

\subsectiontitle{Mohr's Criterion}
Draw Mohr's circles for UTS and UCS, state circle must fit inside circles.\\
Can add torsion test circle centered at origin with radius equal to $\tau_U$.\\
Enveloppe or tangent line around circles.\\
\end{minipage}


\begin{tikzpicture}[remember picture, overlay]
\node[opacity=0.7, anchor=north west] (image) at (0,0.5cm) {\includegraphics[width=0.80in]{image27.png}};
\node[opacity=0.7, anchor=north east] (image) at (7.0cm,0.5cm) {\includegraphics[width=0.50in]{image28.png}};
\end{tikzpicture}
\begin{minipage}{0.95\linewidth}
...
\begin{tikzpicture}
\node[opacity=0.7, anchor=center] (image) at (0,0) {\includegraphics[width=0.80\linewidth]{image25.png}};
...
\vspace{1cm}
\begin{center}
\includegraphics[width=2.5in]{image31.png}
\end{center}
same principle for centroid
\begin{center}
\includegraphics[width=1.5in]{image32.png}
\end{center}
\begin{center}
\includegraphics[width=1.5in]{image33.png}
\end{center}
